...
(cd $GASFLOW && git checkout original_test && git pull && ant clean compile jar)
...
(cd $GASFLOW && git checkout punit_expr && git pull && cp $UI/build/libs/units-inference.jar $GASFLOW/lib/. && ant clean compile jar)
